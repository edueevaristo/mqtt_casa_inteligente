...
static void (*s_message_callback)(const char* topic, const char* data, int len) = NULL;

/** Controle do ar condicionado */
static uint64_t s_temp_baixa_inicio = 0;
static bool s_ar_condicionado_ligado = false;
...
esp_err_t mqtt_system_init(void)
{
    ...
    if (ret != ESP_OK) {
        ...
        ESP_LOGE(TAG, "Falha ao inicializar GPIO");
        return ret;
    }
...
void mqtt_set_message_callback(void (*callback)(const char* topic, const char* data, int len))
{
...
static esp_err_t init_gpio(void)
{
    ESP_LOGI(TAG, "  -> Inicializando GPIOs...");

    gpio_config_t io_conf = {
        .intr_type = GPIO_INTR_DISABLE,
        .mode = GPIO_MODE_OUTPUT,
        .pin_bit_mask = (1ULL << GPIO_LUZES) | (1ULL << GPIO_AR_CONDICIONADO),
        .pull_down_en = 0,
        .pull_up_en = 0,
    };
    ...
    if (ret == ESP_OK)
    {
        gpio_set_level(GPIO_LUZES, 0);
        gpio_set_level(GPIO_AR_CONDICIONADO, 0);
        ESP_LOGI(TAG, "  GPIOs inicializados (Luzes: %d, AR: %d)", GPIO_LUZES, GPIO_AR_CONDICIONADO);
    }
...
static esp_err_t create_tasks(void)
{
    ...
    ret = xTaskCreate(sensor_task, "Sensors",
                      2048, NULL, 2, NULL);
    ...
    ESP_LOGI(TAG, "  Task de sensores criada");
                               ...
                               int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_CONNECTED:
        ...
        mqtt_subscribe_topic(MQTT_TOPIC_LUMINOSIDADE, 1);
    ...
    case MQTT_EVENT_DATA:
        ...
        if (strcmp(topic, MQTT_TOPIC_LUMINOSIDADE) == 0) {
            
            int luminosidade = atoi(data);
            
            if (luminosidade < 3) {

                gpio_set_level(GPIO_LUZES, 1);
                ESP_LOGI(TAG, "As luzes estão LIGADAS (Luminosidade: %d)", luminosidade);
            
            } else {
            
                gpio_set_level(GPIO_LUZES, 0);
                ESP_LOGI(TAG, "As luzes estão DESLIGADAS (Luminosidade: %d)", luminosidade);
            }
        
        } else if (strcmp(topic, "casa/sala/temperatura") == 0) {

            int temperatura = atoi(data);
            uint64_t agora = esp_timer_get_time() / 1000000ULL;
            
            if (temperatura > 23) {

                gpio_set_level(GPIO_AR_CONDICIONADO, 1);
                s_ar_condicionado_ligado = true;

                ESP_LOGI(TAG, "AR LIGADO (Temperatura: %d°C)", temperatura);

            } else if (temperatura < 20) {

                if (s_temp_baixa_inicio == 0) {
                    s_temp_baixa_inicio = agora;

                    ESP_LOGI(TAG, "Temperatura baixa iniciada: %d°C", temperatura);

                } else if (s_ar_condicionado_ligado && (agora - s_temp_baixa_inicio) >= 600) {
                    
                    gpio_set_level(GPIO_AR_CONDICIONADO, 0);
                    s_ar_condicionado_ligado = false;
                    s_temp_baixa_inicio = 0;
                    
                    ESP_LOGI(TAG, "AR DESLIGADO após 10min (temperatura: %d°C)", temperatura);
                }
        ...
        if (s_message_callback) {
            ...
            s_message_callback(topic, data, strlen(data));
        }
...
static void sensor_task(void *pvParameters)
{
    ...
    while (1)
    {
        if (s_mqtt_connected)
        {
            // Sensor de luminosidade (0-10)
            int luminosidade = esp_random() % 11;
            char lum_str[8];
            snprintf(lum_str, sizeof(lum_str), "%d", luminosidade);
            mqtt_publish_data(MQTT_TOPIC_LUMINOSIDADE, lum_str, 0, 1, false);
            
            // Sensor de temperatura (-3 a 45)
            int temperatura = -3 + (esp_random() % 49);
            char temp_str[8];
            snprintf(temp_str, sizeof(temp_str), "%d", temperatura);
            mqtt_publish_data(MQTT_TOPIC_TEMPERATURA, temp_str, 0, 1, false);
            
            ESP_LOGI(TAG, "Sensores: Lum=%d, Temp=%d°C", luminosidade, temperatura);
        }
